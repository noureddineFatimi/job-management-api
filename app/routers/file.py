...
async def ajouter_logo_entreprise(token: Annotated[str, Depends(oauth2_scheme)], file: Optional[UploadFile] = File(None), db: Session = Depends(get_db)):
    if file is None: 
        return {"id_fichier": 1}
    fichier = await process_upload_file(file=file, allowed_types=ALLOWED_IMAGE_TYPES, max_size=MAX_FILE_SIZE, upload_folder=UPLOAD_PATH / "images")
...
async def modifier_logo_entreprise(token: Annotated[str, Depends(oauth2_scheme)], id_entreprise: int, file: UploadFile = File(...), db: Session = Depends(get_db)):
...
def supprimer_logo_entreprise(token: Annotated[str, Depends(oauth2_scheme)], id_entreprise: int, db: Session = Depends(get_db)):
    e = check_entreprise_exist(id_entreprise, db)
    if not e:
        raise HTTPException(status_code = 404, detail = "entreprise_inexistant")
    m = delete_logo(e, db)
