...
def modifier_offre(token: Annotated[str, Depends(oauth2_scheme)], id_offre: int, offre: OffreUpdate, db: Session = Depends(get_db)):
...
def supprimer_offre(token: Annotated[str, Depends(oauth2_scheme)], id_offre: int, db: Session = Depends(get_db)):
    offre = job.check_offre_exist(id_offre, db)
    if not offre: 
        raise HTTPException(status_code = 404, detail = "offre_inexistant")
...
def postuler_a_offre(id_offre: int, candidature: ApplyIn, db : Session =  Depends(get_db)):
    ...
    c = job.creat_save_candidature(offre, candidature, db)
    return c